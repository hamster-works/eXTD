# eXTDDriver Releases

Windows x64 installer releases for eXTDDriver. Choose your language below.

- [English](#english)
- [Deutsch](#deutsch)
- [Francais](#francais)
- [Italiano](#italiano)
- [Espanol](#espanol)
- [Portugues](#portugues)
- [Nederlands](#nederlands)
- [Russkij](#russkij)
- [日本語](#日本語)
- [한국어](#한국어)
- [简体中文](#简体中文)
- [繁體中文](#繁體中文)

## English

Official eXternalTouchDisplay Driver (eXTDDriver) installer for Windows x64.

The downloaded file is an installer for Windows x64.

- Download and install the EXE file.
- The Source Code (zip/tar.gz) files are not required.

[Download the latest release](https://github.com/hamster-works/eXTD/releases/latest)

## Deutsch

Offizielles Installationsprogramm für eXternalTouchDisplay Driver (eXTDDriver) für Windows x64.

Die heruntergeladene Datei ist ein Installationsprogramm für Windows x64.

- Laden Sie die EXE-Datei herunter und installieren Sie sie.
- Die Dateien Source Code (zip/tar.gz) werden nicht benötigt.

[Neueste Version herunterladen](https://github.com/hamster-works/eXTD/releases/latest)

## Francais

Installateur officiel de eXternalTouchDisplay Driver (eXTDDriver) pour Windows x64.

Le fichier téléchargé est un installateur pour Windows x64.

- Téléchargez et installez le fichier EXE.
- Les fichiers Source Code (zip/tar.gz) ne sont pas nécessaires.

[Télécharger la dernière version](https://github.com/hamster-works/eXTD/releases/latest)

## Italiano

Programma di installazione ufficiale di eXternalTouchDisplay Driver (eXTDDriver) per Windows x64.

Il file scaricato è un programma di installazione per Windows x64.

- Scarica e installa il file EXE.
- I file Source Code (zip/tar.gz) non sono necessari.

[Scarica l’ultima versione](https://github.com/hamster-works/eXTD/releases/latest)

## Espanol

Instalador oficial de eXternalTouchDisplay Driver (eXTDDriver) para Windows x64.

El archivo descargado es un instalador para Windows x64.

- Descarga e instala el archivo EXE.
- Los archivos Source Code (zip/tar.gz) no son necesarios.

[Descargar la última versión](https://github.com/hamster-works/eXTD/releases/latest)

## Portugues

Instalador oficial do eXternalTouchDisplay Driver (eXTDDriver) para Windows x64.

O arquivo baixado é um instalador para Windows x64.

- Baixe e instale o arquivo EXE.
- Os arquivos Source Code (zip/tar.gz) não são necessários.

[Baixar a versão mais recente](https://github.com/hamster-works/eXTD/releases/latest)

## Nederlands

Officieel installatieprogramma van eXternalTouchDisplay Driver (eXTDDriver) voor Windows x64.

Het gedownloade bestand is een installatieprogramma voor Windows x64.

- Download en installeer het EXE-bestand.
- De bestanden Source Code (zip/tar.gz) zijn niet nodig.

[Download de nieuwste versie](https://github.com/hamster-works/eXTD/releases/latest)

## Русский

Официальный установщик eXternalTouchDisplay Driver (eXTDDriver) для Windows x64.

Загруженный файл является установщиком для Windows x64.

- Скачайте и установите EXE-файл.
- Файлы Source Code (zip/tar.gz) не требуются.

[Скачать последнюю версию](https://github.com/hamster-works/eXTD/releases/latest)

## 日本語

eXternalTouchDisplay Driver (eXTDDriver)のWindows x64用公式インストーラーです。

ダウンロードするファイルはWindows x64用インストーラーです。

- EXEファイルをダウンロードしてインストールしてください。
- Source Code (zip/tar.gz)は不要です。

[最新リリースをダウンロード](https://github.com/hamster-works/eXTD/releases/latest)

## 한국어

eXternalTouchDisplay Driver (eXTDDriver)의 공식 Windows x64 설치 프로그램입니다.

다운로드하는 파일은 Windows x64용 설치 프로그램입니다.

- EXE 파일을 다운로드하여 설치하세요.
- Source Code (zip/tar.gz) 파일은 필요하지 않습니다.

[최신 릴리스 다운로드](https://github.com/hamster-works/eXTD/releases/latest)

## 简体中文

这是 eXternalTouchDisplay Driver (eXTDDriver) 的 Windows x64 官方安装程序。

下载的文件是适用于 Windows x64 的安装程序。

- 请下载并安装 EXE 文件。
- 不需要 Source Code (zip/tar.gz) 文件。

[下载最新版本](https://github.com/hamster-works/eXTD/releases/latest)

## 繁體中文

這是 eXternalTouchDisplay Driver (eXTDDriver) 的 Windows x64 官方安裝程式。

下載的檔案是適用於 Windows x64 的安裝程式。

- 請下載並安裝 EXE 檔案。
- 不需要 Source Code (zip/tar.gz) 檔案。

[下載最新版本](https://github.com/hamster-works/eXTD/releases/latest)


